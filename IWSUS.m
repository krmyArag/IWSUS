...
switch ipos
    case 5 % en_b
        %% pos1
        pos1=-33.8*exp(-2.29*ratio)+12.76;
        if ratio<0.5
            pos1=-33.8*exp(-2.29*0.5)+12.76;
        end
        [~,z1]=min(abs(z-pos1));
        
        w1=u0*(-1.666*exp(-2.685*ratio)+0.5563);
        if ratio<0.5
            w1=u0*(0.5364*exp(1.577*ratio)-1.084);
        end
        ...
        pos2=H;
        [~,z2]=min(abs(z-pos2));
        w2=u0*(-2.045*exp(-8.675*ratio)-0.2793);
        if ratio<0.5
            w2=u0*(1.724*exp(.5*ratio)-2.521);
        end
        b2=-pos1;c2=w1;a2=(w2-w1)/(pos2-pos1)^2;
        ...
        b3=-0.3;
        b3=-0.3;
        a3=(w2-c3)/(exp(b3*pos2));
        w(z2:end)=a3*exp(b3*z(z2:end))+c3;
    case 6 % en_y
        %% log
        pos1=H;
        [~,z1]=min(abs(z-pos1));
        
        w1=u0*(-.2577*exp(-1.262*ratio)+0.6489);
        if abs(pos1)>1e-5
        ...
        %% exp
        c2=0;
        b2=-0.6522*exp(-3.608*ratio)-0.03685;
        a2=w1/(exp(b2*pos1));
        w(z1:end)=a2*exp(b2*z(z1:end))+c2;
    case 3 % mid_b
        %% pos1
        pos1=H*(-1.8*exp(-1.625*ratio)+0.7463);
        if ratio<1
            pos1=H*(-0.4833*ratio+0.875);
        end
        if ratio<0.5
            pos1=H*(-0.4833*0.5+0.875);
        end
        [~,z1]=min(abs(z-pos1));
        
        w1=u0*(8*exp(-4.531*ratio)-0.3756);
        if ratio<0.5
            w1=u0*(0.3323*ratio+0.2978);
        end
        b1=-pos1;c1=w1;a1=-c1/b1^2;
        w=a1*(z+b1).^2+c1;
        %% pos2
        pos2=H*(0.9415*exp(-3.183*ratio)+0.9158);
        [~,z2]=min(abs(z-pos2));
        
        w2=u0*(3.597*exp(-10.62*ratio)+0.05026);
        if ratio<0.5
            w2=u0*(.2747*exp(.5*ratio)-0.2845);
        end
        b2=-pos1;c2=w1;a2=(w2-w1)/(pos2+b2)^2;
        w(z1:z2)=a2*(z(z1:z2)+b2).^2+c2;
        %% pos3
        pos3=H*(1.316*exp(-4.041*ratio)+1.055);
        [~,z3]=min(abs(z-pos3));
        w3=u0*(0.1807*ratio-0.04776);
        if ratio > 0.5
            w3=u0*(0.9714*exp(-3.458*ratio)-0.1298);
        end
        a3=(w3-w2)/(pos3-pos2);b3=w3-a3*pos3;
        w(z2:z3)=a3*z(z2:z3)+b3;
        %% pos4
        pos4=H*(14.87*exp(-3.06*ratio)-1.253);
        if ratio>0.5
            pos4=H*(14.87*exp(-3.06*0.5)-1.253);
        end
        [~,z4]=min(abs(z-pos4));
        w4=u0*(-0.158*exp(-1.74*ratio)+0.1778);
        b4=-pos4;c4=w4;a4=(w3-c4)/(pos3+b4)^2;
        w(z3:z4)=a4*(z(z3:z4)+b4).^2+c4;
        %% exp
        b5=-0.05;
        a5=w4/exp(b5*pos4);
        w(z4:end)=a5*exp(b5*z(z4:end));
    case 4 % mid_y
        %% log
        pos1=H*(-1.049*exp(-2.552*ratio)+0.5267);
        if pos1 <= 0
            pos1=0;
        end
        [~,z1]=min(abs(z-pos1));
        
        w1=u0*(-1.184*exp(-0.926*ratio)+0.4016);
        if ratio < 1
            w1=u0*(.1457*exp(-1.487*ratio)-0.1004);
        end
        if abs(pos1)>1e-5
            b1=-pos1;c1=w1;a1=-c1/b1^2;
            w=a1*(z+b1).^2+c1;
        ...
        w2=u0*(0.4676*exp(-4.358*ratio)+0.2105);
        if ratio>1
            w2=u0*(-0.2168*exp(-0.6508*ratio)+0.3296);
        end
        
        b2=-pos1;c2=w1;a2=(w2-c2)/(pos2+b2)^2;
        w(z1:z2)=a2*(z(z1:z2)+b2).^2+c2;
        %% pos3
        pos3=H*(6.427*exp(-4.3*ratio)+1.403);
        [~,z3]=min(abs(z-pos3));
        
        w3=u0*(-2.004*exp(-0.07454*ratio)+1.916);
        if ratio<1.5
            w3=u0*(0.003453*exp(2.456*ratio)-0.004888);
        end
        b3=-pos3;c3=w3;a3=(w2-c3)/(pos2+b3)^2;
    ...
    case 1 % out_b
        %% pos1
        pos1=H*(-1.235*exp(-.7073*ratio)+0.8608);
        if ratio<1
            pos1=H*(-1.163*ratio+1.415);
        end
        if ratio<0.5
            pos1=H*(-1.163*0.5+1.415);
        end
        [~,z1]=min(abs(z-pos1));
        
        w1=u0*(3.574*exp(-2.712*ratio)-0.3564);
        if ratio<0.5
            w1=u0*(0.9696*exp(.5*ratio)-0.6803);
        end
        b1=-pos1;c1=w1;a1=-c1/b1^2;
        w=a1*(z+b1).^2+c1;
        
        %% pos2
        pos2=H*(1.195*exp(-2.207*ratio)+0.8687);
        [~,z2]=min(abs(z-pos2));
        
        w2=u0*(-0.1846*exp(-2.444*ratio)+0.3174);
        
        b2=-pos1;c2=w1;a2=(w2-w1)/(pos2+b2)^2;
        w(z1:z2)=a2*(z(z1:z2)+b2).^2+c2;
        
        %% pos3
        pos3=H*(1.143*exp(-2.615*ratio)+1.24);
        [~,z3]=min(abs(z-pos3));
        
        w3=u0*(-0.2338*exp(-0.7722*ratio)+0.4424);
    ...
    case 2 % out _y
        %% log
        pos1=H*(-.5533*exp(-1.439*ratio)+0.3861);
        if pos1 <= 0
            pos1=0;
        end
        [~,z1]=min(abs(z-pos1));
        
        w1=u0*(0.2934*exp(-3.258*ratio)-0.1289);
        if abs(pos1)>1e-5
            a1=w1/pos1;
            w=a1*z;
        else
